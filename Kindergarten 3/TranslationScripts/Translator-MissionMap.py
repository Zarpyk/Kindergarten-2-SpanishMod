...
SYSTEM_PROMPT = (
    ...
    "# Instructions:\n"
    "- Translate the sentence to neutral Spanish.\n"
    "- Keep placeholders like {0}, {1}, etc. exactly as they are.\n"
    ...
    while i < len(lines):
        ...
        if line.startswith("[Entry ID:"):
            ...
            if i + 1 < len(lines):
                ...
                entries[current_id] = translation
                i += 2
            else:
                i += 1
        else:
...
def process_csv_translation(input_path, output_path):
    ...
    for i, row in enumerate(reader):
        if i in translations:
            row["Traducción [es]"] = translations[i]
        else:
            row["Traducción [es]"] = row["Default"]
...
input_csv_path = r'C:\Development\Projects\Kindergarten-2-SpanishMod\Kindergtarten 3\TranslationScripts\MissionMap.csv'
output_csv_path = r'C:\Development\Projects\Kindergarten-2-SpanishMod\Kindergtarten 3\TranslationScripts\MissionMap_es_new.csv'

process_csv_translation(input_csv_path, output_csv_path)
