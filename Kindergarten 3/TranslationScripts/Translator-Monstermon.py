...
SYSTEM_PROMPT = (
    "You are an English to Spanish translator tasked with translating the names and descriptions of Monstermon creatures.\n\n"
    "# Instructions:\n"
    "- Translate the NAME (first line) with creativity, preserving humor, tone, and wordplay.\n"
    "- Translate the DESCRIPTION (second line) keeping the joke or weirdness intact.\n"
    "- If no meaning in spanish, add explanation in parenthesis"
    "- Do NOT translate 'goo'.\n"
    "- Maintain punctuation and quotation marks.\n"
    "- If a pun exists, adapt it to a Spanish equivalent that fits the creature.\n\n"
    "# Output Format:\n"
...
def process_monstermon_csv(input_path, output_path):
    ...
    for i, row in enumerate(reader):
        if i in translations:
            ...
            row["Descripción traducida [es]"] = translated_desc
        else:
            row["Nombre traducido [es]"] = row["Field"]
            row["Descripción traducida [es]"] = row["Default"]
...
input_csv_path = r'C:\Development\Projects\Kindergarten-2-SpanishMod\Kindergtarten 3\TranslationScripts\Monstermon_es.csv'
output_csv_path = r'C:\Development\Projects\Kindergarten-2-SpanishMod\Kindergtarten 3\TranslationScripts\Monstermon_es_new.csv'

process_monstermon_csv(input_csv_path, output_csv_path)
